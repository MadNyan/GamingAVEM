...
def _global_2D_correlation(org_img: np.ndarray, pred_img: np.ndarray) -> float:
    x_mean = np.mean(np.array(org_img))
    y_mean = np.mean(np.array(pred_img))
    ...
    for i in range(org_img.shape[0]):
        for j in range(org_img.shape[1]):
            x = org_img[i][j][0] - x_mean
            y = pred_img[i][j][0] - y_mean
            numerator = numerator + (x * y)
            x_denominator = x_denominator + math.pow(x, 2)
            y_denominator = y_denominator + math.pow(y, 2)
...
def fsm(org_img: np.ndarray, pred_img: np.ndarray, a=5.0, b=3.0, c=7.0, e=0.01) -> float:
    """
    Feature-based structural measure, based on Feature-based similarity index (FSIM) and Structural SIMularity index (SSIM)
    ...
    Args:
        ...
        b -- constant
        c -- constant
        e -- added to balance the quotient and avoid division by zero
    """
    _assert_image_shapes_equal(org_img, pred_img, "FSM")

    org_gray = _mean_gray_scale(org_img)
    pred_gray = _mean_gray_scale(pred_img)

    low_threshold = 30
    high_threshold = 150
    org_edges = cv2.Canny(org_gray, low_threshold, high_threshold)
    pred_edges = cv2.Canny(pred_gray, low_threshold, high_threshold)

    org_edges = np.array(org_edges).reshape(org_edges.shape[0],org_edges.shape[1],1)
    pred_edges = np.array(pred_edges).reshape(pred_edges.shape[0],pred_edges.shape[1],1)

    ssim_val = ssim(org_edges, pred_edges)
    fsim_val = fsim(org_edges, pred_edges)
    r_val = _global_2D_correlation(org_edges, pred_edges)

    numerator = ((a + c) * r_val * fsim_val) + (b * r_val) + e
    denominator = ((a + b) * ssim_val) + (c * r_val * fsim_val) + e
