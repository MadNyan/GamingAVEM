...
from src.model.uniSpeech import UniSpeechForSpeechClassification, UniSpeechClassificationHead
...
class trainerUniSpeech(trainer):
    ...
    def get_dataset(self, x_train: np.ndarray, x_val: np.ndarray, x_test: np.ndarray, y_train: np.ndarray, y_val: np.ndarray, y_test: np.ndarray, num_classes: int, batch_size: int = 4, model_id: str = 'microsoft/unispeech-1350-en-353-fr-ft-1h'):
    ...
    def get_feature(self, model: nn.Module, x_train: np.ndarray, x_val: np.ndarray, x_test: np.ndarray, model_id: str = 'microsoft/unispeech-1350-en-353-fr-ft-1h'):
        x_train_input_values, x_train_attention_mask, x_val_input_values, x_val_attention_mask, x_test_input_values, x_test_attention_mask = self.get_dataset_x(x_train, x_val, x_test, model_id)
    ...
    def get_dataset_x(self, x_train: np.ndarray, x_val: np.ndarray, x_test: np.ndarray, model_id: str = 'microsoft/unispeech-1350-en-353-fr-ft-1h'):
        ...
        x_train = feature_extractor(x_train, sampling_rate=target_sampling_rate, return_tensors='pt', padding=True)
        x_val = feature_extractor(x_val, sampling_rate=target_sampling_rate, return_tensors='pt', padding=True)
        x_test = feature_extractor(x_test, sampling_rate=target_sampling_rate, return_tensors='pt', padding=True)   
    ...
    def load_model(self, path: str = ''):
        ...
        if is_load:
            self.model.classifier = UniSpeechClassificationHead(self.num_classes)
            self.init_loss()
            self.init_optimizer_and_scheduler()

    def init_model(self, model_id: str = 'microsoft/unispeech-1350-en-353-fr-ft-1h'):
        # config
        ...
        self.model = UniSpeechForSpeechClassification.from_pretrained(
            model_id,
            config=config,
        )
